...
{
	/// <summary>
	/// Disposable non thread-safe cache of <typeparamref name="V"/> items which also
	...
	/// </remarks>
	public class DisposableSequentialMRUCache<K, V> : SequentialMRUCache<K, V>, IDisposable
		where V : class, IDisposable
	{
		...
		public DisposableSequentialMRUCache(Func<K, V> itemCreator, int maxCount = 1024)
		...
		/// <summary>
		/// Calls <see cref="SequentialMRUCache{K, V}.Clear"/> to evict and dispose all items in the cache.
		/// </summary>
